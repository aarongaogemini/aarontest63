...
from app.domain.models.app_config import AppConfig, LLMConfig, AgentConfig, MCPConfig, A2AConfig, A2AServerConfig, SkillConfig, SkillsConfig
...
from app.interfaces.schemas.app_config import ListMCPServerItem, ListA2AServerItem, ListSkillItem
...
class AppConfigService:
    ...
    async def get_skills(self) -> List[ListSkillItem]:
        """获取Skills技能列表"""
        # 1.获取当前的应用配置
        app_config = await self._load_app_config()
        ...
        # 2.组装响应结构
        skills = []
        for skill in app_config.skills_config.skills:
            skills.append(ListSkillItem(
                id=skill.id,
                name=skill.name,
    ...
    async def create_skill(self, name: str, description: str, content: str = "") -> SkillsConfig:
        """根据传递的名字、描述和内容新增一个Skill技能"""
        # 1.获取当前的应用配置
        app_config = await self._load_app_config()
        ...
        app_config.skills_config.skills.append(skill_config)

        # 3.调用数据仓库更新
        self.app_config_repository.save(app_config)
        return app_config.skills_config

    async def set_skill_enabled(self, skill_id: str, enabled: bool) -> SkillsConfig:
        """根据传递的id+enabled更新Skill启用状态"""
        # 1.获取当前的应用配置
        app_config = await self._load_app_config()

        # 2.计算需要更新位置的索引并判断是否存在
        idx = None
        for item_idx, item in enumerate(app_config.skills_config.skills):
            if item.id == skill_id:
                idx = item_idx
                break
        if idx is None:
            raise NotFoundError(f"该Skill技能[{skill_id}]不存在，请核实后重试")

        # 3.如果存在则更新数据
        app_config.skills_config.skills[idx].enabled = enabled
        self.app_config_repository.save(app_config)
        return app_config.skills_config

    async def delete_skill(self, skill_id: str) -> SkillsConfig:
        """根据传递的id删除指定的Skill技能"""
        # 1.获取当前的应用配置
        app_config = await self._load_app_config()
        ...
        # 3.删除Skill技能
        del app_config.skills_config.skills[idx]
        self.app_config_repository.save(app_config)
        return app_config.skills_config
