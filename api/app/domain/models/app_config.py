...
class SkillConfig(BaseModel):
    ...
    content: str = ""  # 技能的具体提示词/指令内容，会被注入到Agent的系统提示词中
...
class SkillsConfig(BaseModel):
    ...
    def get_enabled_skills_prompt(self) -> str:
        """获取所有已启用技能的提示词文本，用于注入到Agent的系统提示词中"""
        enabled_skills = [s for s in self.skills if s.enabled]
        if not enabled_skills:
            return ""

        skills_text = "\n<skills>\n你拥有以下技能，请在规划和执行任务时充分利用它们：\n"
...
class AppConfig(BaseModel):
    """应用配置信息，包含Agent配置、LLM提供商配置、MCP配置、A2A配置、Skills配置"""
    llm_config: LLMConfig  # 语言模型配置
    agent_config: AgentConfig  # Agent通用配置
    mcp_config: MCPConfig  # MCP服务配置
    a2a_config: A2AConfig  # A2A服务配置
    skills_config: SkillsConfig = Field(default_factory=SkillsConfig)  # Skills技能配置
